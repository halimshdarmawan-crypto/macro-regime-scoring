...
export async function onRequestGet(context) {
  ...
  if (!preset) {
    return new Response(
      JSON.stringify({ error: `Indikator tidak dikenal: ${path}`, available: Object.keys(PRESETS) }),
  ...
  if (!env.FRED_API_KEY) {
    return new Response(
      JSON.stringify({ error: "FRED_API_KEY belum diset sebagai Cloudflare Pages secret" }),
  ...
  if (!fredResponse.ok) {
    ...
    return new Response(
      JSON.stringify({ error: "FRED mengembalikan error", status: fredResponse.status, detail }),
      { status: 502, headers: { "content-type": "application/json" } }
    );
  }
